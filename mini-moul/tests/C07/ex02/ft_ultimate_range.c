...
int	main(void)
{
	...
	t_test tests[] = {
		...
		{
			.desc = "ft_ultimate_range with both min and max equal to
				- 2147483648 ",
						.min = -2147483648,
			.max = -2147483648,
			.range = NULL,
...
int	run_tests(t_test *tests, int count)
{
	...
	for (i = 0; i < count; i++)
	{
		expected_size = (tests[i].max - tests[i].min);
		range_size = ft_ultimate_range(&result, tests[i].min, tests[i].max);
		if (tests[i].expected_range == NULL && result == NULL)
		{
			printf("  " GREEN CHECKMARK GREY " [%d] %s Expected NULL,
					got NULL\n" DEFAULT, i + 1, tests[i].desc);
		}
		else if (tests[i].expected_range == NULL || result == NULL)
		{
			printf("    " RED "[%d] %s Expected %p, got %p\n" DEFAULT, i + 1,
					tests[i].desc, tests[i].expected_range, result);
			error--;
		}
