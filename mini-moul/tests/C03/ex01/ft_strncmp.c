...
int	run_tests(t_test *tests, int count)
{
	...
	for (i = 0; i < count; i++)
	{
		...
		if ((result < 0 && tests[i].expected >= 0) || (result > 0
				&& tests[i].expected <= 0) || (result == 0
				&& tests[i].expected != 0))
		{
			printf(RED " [%d] %s: expected %s, got %d\n" DEFAULT, i + 1,
					tests[i].desc,
					(tests[i].expected < 0 ? "negative value" : (tests[i].expected > 0 ? "positive value" : "0")),
					result);
			error -= 1;
		}
		else
		{
			printf("  " GREEN CHECKMARK GREY " [%d] %s: expected %s, got
