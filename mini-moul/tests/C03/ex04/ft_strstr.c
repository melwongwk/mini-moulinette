...
int	run_tests(t_test *tests, int count)
{
	...
	for (i = 0; i < count; i++)
	{
		result = ft_strstr(tests[i].str, tests[i].find);
		if ((result == NULL && tests[i].expected_output != NULL)
			|| (result != NULL && strcmp(result,
					tests[i].expected_output) != 0))
		{
			printf("    " RED "[%d] %s Expected output \"%s\", got \"%s\"\n", i
					+ 1, tests[i].desc, tests[i].expected_output, result);
			error -= 1;
		}
		else
		{
			printf("  " GREEN CHECKMARK GREY " [%d] %s Expected output \"%s\",
					got \"%s\"\n" DEFAULT,
					i + 1,
					tests[i].desc,
