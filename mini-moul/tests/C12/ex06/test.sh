...
readonly assignment_name="ft_list_clear"
...
main() {
	...
	if ! run_norminette "$PROJECT_DIR/$assignment"; then
		...
		rm -f "$SCRIPT_DIR/ft_list_clear.c" "$SCRIPT_DIR"/*.h
		exit 8
	fi

	# Compile test and source
	if ! cc -o "$SCRIPT_DIR/test_exe" "$SCRIPT_DIR/test.c" "$SCRIPT_DIR/ft_list_clear.c" 2> "$SCRIPT_DIR/log"; then
		cat "$SCRIPT_DIR/log"
		rm -f "$SCRIPT_DIR/ft_list_clear.c" "$SCRIPT_DIR"/*.h "$SCRIPT_DIR"/test_exe "$SCRIPT_DIR"/log
